...
#this is motion controller main file for real car
...
def laserCallback(laser_data):
    #distance_front = laser_data.ranges[170:190]
    distance_front = laser_data.ranges[20:190]
...
def main():
    ...
    # EKF localization data
    rospy.Subscriber("/slam_out_pose", PoseStamped, callback_odom)
    laser_subscriber = rospy.Subscriber("/scan", LaserScan, laserCallback, queue_size=1)
    pub_speed = rospy.Publisher('/cmd_vel', Twist, queue_size=1) #publish to cmd_vel

    #some predefined global via points
    ax = [0, 3, 5, 5.5, 5.7, 6, 18, 19, 19, 17, 10]
    ay = [0, 0, 0, -0.2, -4.3, -8, -8.8, -8.6, 0.1, 0.4, 0.4]

    ax = [0, 2.2, 2.5, 2.7, 2.75, 2.93, 3.78, 6]
    ay = [0, -0.0, 0, 0.09, 0.53, 0.7, 0.8, 1.0]
    ...
    while not rospy.is_shutdown() and last_idx > target_idx:
        if target_idx >= last_idx:
            ...
            if obstacle_found == False:
                ...
                #di = np.clip(di, -0.1, 0.1)
                if di > 1.5:
                    di = 1.5
                elif di<-1.5:
                    di = -1.5

                rospy.loginfo("v:{}, w:{}".format(round(state.v, 1), round(di, 1)))
